        ...
        <div class="card card-custom gutter-b">
            ...
            <div class="card-body">
                <table class="table table-striped">
                    <tr>
                        <th>الاسم</th>
                        <td>{{$my_reservation->child_name}}</td>
                    </tr>
                    <tr>
                        <th>اسم الاب</th>
                        <td>{{$my_reservation->father_name2}}</td>
                    </tr>
                    <tr>
                        <th>اسم البرنامج</th>
                        <td>{{$my_reservation->program_name_ar}}</td>
                    </tr>
                    <tr>
                        <th>سعر البرنامج</th>
                        <td>{{$my_reservation->price * $coins->dollar}} {{$coins->coins_name_ar}}</td>
                        <?php $total_price +=  ($my_reservation->price * $coins->dollar);?>
                    </tr>
                    ...
                    <tr>
                        <th>حالة الحجز</th>
                        <td>
                            {{$my_reservation->reservationStatus_status_ar}}
                            <form action="{{URL('facility_m/programChildren/update_reservattions')}}/{{$my_reservation->childProg_id}}" method="post">
                                @csrf
                                <div class="form-group">
                                    <select class="form-control" id="exampleSelect1" name="id_reservation_statuses">
                                        @foreach ($reservationStatus as $status)
                                            <option @if($status->id == $my_reservation->reservationStatus_id) selected @endif value="{{$status->id}}"> {{$status->status_ar}}</option>
                                        @endforeach
                    ...
                    <tr>
                        <th>تاريخ الحجز</th>
                        <td>{{$my_reservation->created_at}}</td>
                    </tr>
        ...
        <div class="card">
            <div class="card-body p-9">
                ...
                <div class="fs-6 d-flex justify-content-between my-4">
                    ...
                    <!--end::Svg Icon-->{{$my_reservation->price * $coins->dollar}} {{$coins->coins_name_ar}}</div>
                </div>
